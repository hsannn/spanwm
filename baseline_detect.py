...
- LTW: the selector replays its gating decision over the text (entropy from the
  BASE LM -> not model-free) and the z test runs over the selected tokens only;
  exact binomial p over (green, scored) as everywhere else. LTW is generated by
  ltw_run.py (its own decoding loop), not by baseline_embed.py -- this script
  just re-scores that jsonl under the shared metric block, which is what makes
  outputs/attacked/ltw/*.jsonl scoreable with --column attacked_text.

Detection needs only the tokenizer (model=None) except for SWEET/EWD/LTW, which
compute per-token entropy from the generation model; GPU strongly recommended
for KGW (per-token randperm over the vocab).

Note: the KGW-family greenlist is a seeded randperm, so detection must run on
the same GPU family as generation (CPU vs CUDA, and even different CUDA archs,
give different permutations from the same seed).
...
# algorithms whose detection needs the generation model loaded (per-token entropy)
...
class LTWScorer:
    """LTW (Learning to Watermark, NeurIPS 2025), vendored in watermark/ltw/.

    The selector network replays its gating decision token by token (entropy
    from the base LM + SimCSE context + running watermarked ratio) and the
    green test runs over the selected tokens only. z from the detector,
    p = P(Binom(N, gamma) >= G) as in every other row.

    Prompt handling deviates from the other scorers on purpose. LTW's prefix is
    not merely "where scoring starts": it also seeds the selector's semantic
    context (the k tokens before the first scored position) and, for the ltw1
    variant, the green-list seed of the first scored token. So when the text
    still carries the prompt (watermarked/unwatermarked columns) the FULL text
    is scored with the prompt as prefix -- identical to ltw_run.py, which is
    what the reported LTW numbers come from. When it does not (attacked_text
    after a paraphrase, or natural_text) it falls back to main()'s
    prompt-stripped text with a 1-token prefix, so LTW is scored over the same
    region as every other method. `--truncation` therefore only affects the
    fallback path.

    k, gamma and the variant MUST match generation: the selector replays the
    gate, so a different k reproduces a different set of scored tokens.
    """

    wants_prompt = True

    def __init__(self, config_path, model, tokenizer, device, variant=None):
        from watermark.ltw.watermark import Detector

        with open(config_path) as f:
            cfg = json.load(f)
        self.variant = variant or cfg["variant"]
        self.gamma = float(cfg["gamma"])
        self.z_threshold = float(cfg["z_threshold"])
        self.tokenizer = tokenizer
        self.device = device
        self.det = Detector(
            vocab=list(range(model.config.vocab_size)),  # logits dim, not len(tokenizer)
            gamma=self.gamma, delta=float(cfg["delta"]),
            seeding_scheme="simple_1", tokenizer=tokenizer, model=model,
            checkpoint_path=cfg["checkpoint"],
            semantic_model_path=cfg["semantic_model"],
            z_threshold=self.z_threshold,
            k=int(cfg["k"]),                       # same window as generation
            embed_unigram_wm=(self.variant == "ltw0"),
        )

    def _ids(self, text):
        return self.tokenizer(text, return_tensors="pt")["input_ids"][0].to(self.device)

    def score(self, text, prompt=None, raw_text=None):
        if prompt and raw_text and raw_text.startswith(prompt):
            ids, prefix_len = self._ids(raw_text), len(self._ids(prompt))
        else:
            ids, prefix_len = self._ids(text), 1
        if len(ids) - max(1, prefix_len) < 5:
            return None
        with torch.no_grad():
            r = self.det.detect(tokenized_text=ids, tokenized_prefix=ids[:prefix_len],
                                return_prediction=False)
        if r.get("invalid"):
            return None
        n = int(r.get("num_tokens_scored", 0))
        g = int(r.get("num_green_tokens", 0))
        if n < 1:
            # selector gated nothing -> no evidence; the detector reports z=-100
            return {"z": -100.0, "p": 1.0, "n": 0, "green": 0, "green_frac": 0.0,
                    "wm_frac": 0.0, "decision": False}
        z = float(r["z_score"])
        return {"z": z, "p": float(binom.sf(g - 1, n, self.gamma)), "n": n,
                "green": g, "green_frac": g / n,
                "wm_frac": float(r.get("watermarking_fraction", 0.0)),
                "decision": bool(z > self.z_threshold)}
...
def main() -> None:
    ...
    if args.algorithm in STANDALONE:
        # not registered in AutoWatermark; its own vendored detector
        scorer = LTWScorer(config_path, det_model, tokenizer, device, variant=args.variant)
        return run_eval(args, config_path, scorer)
    ...
    return run_eval(args, config_path, scorer_cls(watermark, tokenizer, device))


def run_eval(args, config_path, scorer) -> None:
    with open(args.input) as f:
        records = [json.loads(line) for line in f if line.strip()]

    # scorers that own their prompt handling also get the untruncated text
    wants_prompt = getattr(scorer, "wants_prompt", False)

    def score(text, prompt=None, raw_text=None):
        if not text:
            return None
        return scorer.score(text, prompt, raw_text) if wants_prompt else scorer.score(text)

    neg_key = f"{args.negative}_text"
    pos_res, neg_res = [], []
    n_neg = 0
    for r in records:
        raw_pos = r[args.column]
        pos_text = strip_prompt(raw_pos, r["prompt"], args.truncation)
        pos_res.append(score(pos_text, r["prompt"], raw_pos))

        raw_neg = r.get(neg_key, "")
        neg_text = raw_neg
        if args.negative == "unwatermarked":
            neg_text = strip_prompt(raw_neg, r["prompt"], args.truncation)
        if neg_text:
            n_neg += 1
            # natural_text never carried the prompt -> no prefix to hand over
            neg_res.append(score(neg_text, r["prompt"] if args.negative == "unwatermarked" else None,
                                 raw_neg))
        else:
            neg_res.append(None)
    ...
    if pos_ok and "green_frac" in pos_ok[0]:
        ...
        if "wm_frac" in pos_ok[0]:  # LTW: fraction of tokens the selector gated
            print(f"selected frac   : pos {np.mean([x['wm_frac'] for x in pos_ok]):.3f}   "
                  f"neg {np.mean([x['wm_frac'] for x in neg_ok]):.3f}")
    elif pos_ok:
    ...
    with open(scores_out, "w") as f:
        json.dump({
            ...
            "variant": variant, "column": args.column,
