"""SpanWM v4 detection + metrics (base + few-shot infilling variant).

Same metrics as spanwm_detect.py (AUROC / mean z / mean exact-p / TPR@FPR),
but reconstructs + scores with SpanWMV4 (constituent detection). Detection needs
only the tokenizer (model=None).

Run:
    python spanwm_detect_v4.py --input outputs/spanwm_v4_c4_n200.jsonl
    python spanwm_detect_v4.py --input outputs/spanwm_v4_c4_n200.jsonl --negative natural
"""
...
def main() -> None:
    ...
    print(f"samples         : {n}   negative class: {args.negative}   [v4: few-shot infilling]")
