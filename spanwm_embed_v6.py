"""SpanWM v6 embedding: multi-span (several small watermarked windows).

Same flow as spanwm_embed_v5.py but uses SpanWMV6. v3/v4/v5 files untouched.

Run (GPU node, dlmwm env):
    python spanwm_embed_v6.py --dataset c4 --num_samples 200
"""
...
def main() -> None:
    ...
    ap.add_argument("--config", default="config/SpanWM_v6.json")
    ap.add_argument("--output", default=None)
    ap.add_argument("--max_new_tokens", type=int, default=200)
    args = ap.parse_args()

    output = args.output or f"outputs/spanwm_v6_{args.dataset}_n{args.num_samples}.jsonl"
    ...
    watermark = SpanWMV6(args.config, transformers_config)
